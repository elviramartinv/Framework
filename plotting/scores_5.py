...
for sample in samples:
    ...
    for tag in taggers_loop:
        ...
        df1 = df1.Filter("RecoJet_pt.size() == 5")
...
plt.savefig(f"output/{sample_name}/scores_5jets/firstjet_scores.png", dpi=300)
...
plt.xlim(-0.2,2.2)
plt.legend(handles=legend_elements, loc='upper right')
plt.savefig(f"output/{sample_name}/scores_5jets/secondjet_scores.png", dpi=300)
...
for sample in samples:
    ...
    for tag in taggers_loop:
        ...
        scores = df.AsNumpy([f"ThirdJet_{tag}"])
        color = tagger_colors[tag]
...
plt.xlim(-0.2,2.2)
plt.legend(handles=legend_elements, loc='upper right')
plt.savefig(f"output/{sample_name}/scores_5jets/thirdjet_scores.png", dpi=300)
...
for sample in samples:
    ...
    for tag in taggers_loop:
        ...
        scores = df.AsNumpy([f"FourthJet_{tag}"])
        color = tagger_colors[tag]
        plt.hist(scores[f"FourthJet_{tag}"], bins=bins, alpha=0.2, histtype='stepfilled', color=color, label=f'{tag}')
        plt.hist(scores[f"FourthJet_{tag}"], bins=bins, alpha=1, histtype='step', color=color, label=f'{tag}')
        legend_label = "HHBtagScore_v3" if tag == "HHBtagScore" else tag
        if legend_label not in [elem.get_label() for elem in legend_elements]:
            legend_elements.append(Patch(facecolor=color, edgecolor=color, alpha=1, linewidth=2, label=legend_label))
plt.xlabel("Score")
...
plt.xlim(-0.2,2.2)
plt.legend(handles=legend_elements, loc='upper right')
plt.savefig(f"output/{sample_name}/scores_5jets/fourthjet_scores.png", dpi=300)
...
for sample in samples:
    ...
    for tag in taggers_loop:
        ...
        df = df.Define('RecoJet_idx', 'CreateIndexes(RecoJet_pt.size())')
        df = df.Define(f'idx_jets_{tag}', f'ReorderObjects(RecoJet_{tag}, RecoJet_idx)')
        df = df.Define(f"FifthJet_{tag}", f"RecoJet_{tag}[idx_jets_{tag}[4]]")
        scores = df.AsNumpy([f"FifthJet_{tag}"])
        color = tagger_colors[tag]
        plt.hist(scores[f"FifthJet_{tag}"], bins=bins, alpha=0.2, histtype='stepfilled', color=color, label=f'{tag}')
        plt.hist(scores[f"FifthJet_{tag}"], bins=bins, alpha=1, histtype='step', color=color, label=f'{tag}')
        legend_label = "HHBtagScore_v3" if tag == "HHBtagScore" else tag
        if legend_label not in [elem.get_label() for elem in legend_elements]:
            legend_elements.append(Patch(facecolor=color, edgecolor=color, alpha=1, linewidth=2, label=legend_label))
plt.xlabel("Score")
plt.ylabel("Normalized Events") 
plt.title("Fifth Jet scores")
plt.yscale('log')
plt.xlim(-0.2,2.2)
plt.legend(handles=legend_elements, loc='upper right')
plt.savefig(f"output/{sample_name}/scores_5jets/fifthjet_scores.png", dpi=300)